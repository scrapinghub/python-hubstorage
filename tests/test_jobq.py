...
def test_simple_botgroups(hsclient, hsproject):
    hsproject.settings['botgroups'] = ['g1']
    hsproject.settings.save()
    pq = hsproject.jobq
    hq = hsclient.jobq
    q1 = pq.push(TEST_SPIDER_NAME)
    assert hq.start(botgroup='g3') is None
    assert apipoll(hq.start, botgroup='g1')['key'] == q1['key']

# FIXME
#@unittest.skipUnless(EXCLUSIVE, "test requires exclusive"
#    " (without any active bots) access to HS. Set EXCLUSIVE_STORAGE"
#    " env. var to activate")
def test_botgroups(hsclient, hsproject):
    hsproject.settings['botgroups'] = ['g1', 'g2']
    hsproject.settings.save()
    pq = hsproject.jobq
    hq = hsclient.jobq
    q1 = pq.push(TEST_SPIDER_NAME)
    q2 = pq.push(TEST_SPIDER_NAME)
    q3 = pq.push(TEST_SPIDER_NAME)
    assert hq.start() is None
    assert hq.start(botgroup='g3') is None
    assert apipoll(hq.start, botgroup='g1')['key'] == q1['key']
    assert apipoll(hq.start, botgroup='g2')['key'] == q2['key']

    # cleanup project botgroups, q3 must be polled only by generic bots
    del hsproject.settings['botgroups']
    hsproject.settings.save()
    q4 = pq.push(TEST_SPIDER_NAME)
    assert hq.start(botgroup='g1') is None
    assert hq.start(botgroup='g2') is None
    assert hq.start(botgroup='g3') is None
    assert hq.start()['key'] == q3['key']
    assert hq.start()['key'] == q4['key']

    hsproject.settings['botgroups'] = ['python-hubstorage-test']
    hsproject.settings.save()
